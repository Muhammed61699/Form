...
if (empty($error)) {
 $sql= "SELECT * FROM   Form WHERE email='$email'";
  ...
  if($result){
    ...
    if ($rows == 0){
        $sql = "INSERT INTO gg(FirstName,LastName,email, phone,birthday,status,family,gender,language,Nationality,Role,Comment) VALUES 
        ('$FirstName','$LastName','$email','$phone','$birthday','$status','$family','$gender','$language','$Nationality','$Role','$Comment')";
   $result = mysqli_query($conn,$sql);
 if ($result){
...
<?php if (!empty($error)){?><h1 class="Done"><?php echo $error;?></h1><?php }?>
